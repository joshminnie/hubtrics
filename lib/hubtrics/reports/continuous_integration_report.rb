...
module Hubtrics
  module Reports
    class ContinuousIntegrationReport < Hubtrics::Reports::Base
      ...
      def template
        @template ||= Liquid::Template.parse(
          File.read(File.expand_path('../templates/continuous_integration_report.md.liquid', __dir__))
        )
      end
      ...
      def title
        "Hubtrics: Pull Request Continuous Integration Report for #{Date.today}"
      end
      ...
      def files
        { 'continuous_integration.md' => { content: report } }
      end
    end
  end
end
